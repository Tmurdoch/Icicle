...
void RenderManager::initVulkan() {
    ...
    RenderManager::createSurface();
    RenderManager::pickPhysicalDevice();
    RenderManager::createLogicalDevice();
    RenderManager::createSwapChain();
    RenderManager::createImageViews();
    RenderManager::createRenderPass();
    RenderManager::createDescriptorSetLayout();
    RenderManager::createGraphicsPipeline();
    RenderManager::createCommandPool();
    RenderManager::createDepthResources();
    RenderManager::createFramebuffers();
    RenderManager::createTextureImage();
    RenderManager::createTextureImageView();
    RenderManager::createTextureSampler();
    RenderManager::createVertexBuffer();
    RenderManager::createIndexBuffer();
    RenderManager::createUniformBuffers();
    RenderManager::createDescriptorPool();
    RenderManager::createDescriptorSets();
    RenderManager::createCommandBuffers();
    RenderManager::createSyncObjects();
}
...
void RenderManager::createInstance() {
    ...
    appInfo.pApplicationName = "RenderManager";
...
void RenderManager::createGraphicsPipeline() {
    //TODO: extract this and clean it up (fix relative path and project organization)
    ...
    /*The vertex shader processes each vertex's attributes, transforming them
    from object space to clip space. This stage performs operations such as model
    transformations, view transformations, and projection transformations.*/
    auto vertShaderCode = loadShader("../shaders/vert.spv");
    /*Fragments generated by rasterization pass through the optional fragment shader.
    If a fragment shader is present, it can perform per-fragment operations such as
    texture mapping, lighting calculations, and other shading effects.*/
    auto fragShaderCode = loadShader("../shaders/frag.spv");

    //wrap in shader module, these objects are consumed when passed to command
...
void RenderManager::createTextureImage() {
    ...
    unsigned char* pixels = load_image(&texWidth, &texHeight, &texChannels);
...
void RenderManager::copyBufferToImage(
                        VkBuffer buffer, 
                        VkImage image, 
                        uint32_t width, 
                        uint32_t height) {
...
void RenderManager::createVertexBuffer() {
    ...
    // Step 1: Create a staging buffer for vertex data transfer
    VkBuffer stagingBuffer;
    VkDeviceMemory stagingBufferMemory;
    createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingBufferMemory);

    // Step 2: Copy vertex data to the staging buffer
    void* data;
    vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
    memcpy(data, vertices.data(), (size_t)bufferSize);
    vkUnmapMemory(device, stagingBufferMemory);

    // Step 3: Create a device-local vertex buffer
    createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vertexBuffer, vertexBufferMemory);

    // Step 4: Copy data from the staging buffer to the device-local vertex buffer
    copyBuffer(stagingBuffer, vertexBuffer, bufferSize);

    // Step 5: Destroy the staging buffer after data transfer is complete
    vkDestroyBuffer(device, stagingBuffer, nullptr);
    vkFreeMemory(device, stagingBufferMemory, nullptr);
}
...
void RenderManager::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
...
void RenderManager::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void RenderManager::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    renderPassInfo.renderPass = RenderManager::renderPass;
    renderPassInfo.framebuffer = RenderManager::swapChainFramebuffers[imageIndex];
    renderPassInfo.renderArea.offset = { 0, 0 };
    renderPassInfo.renderArea.extent = RenderManager::swapChainExtent;
...
int RenderManager::startUp() {
    std::cout << "calling renderman.run()" << std::endl;
    try {
        RenderManager::run();
    }
